...
function formatAgentMessage(value){
  let normalized=String(value).replace(/\s+(\d+)\.\s+(?=\*\*)/g,"\n$1. ");
  if((normalized.match(/\s+\(\d+\)\s+/g)||[]).length>=2)normalized=normalized.replace(/\s+\((\d+)\)\s+/g,"\n$1. ");
  const blocks=normalized.split(/```/);
  return blocks.map((block,index)=>{if(index%2){const lines=block.replace(/^\w+\n/,"");return `<pre class="message-code"><code>${escapeHtml(lines.trim())}</code></pre>`;}let html=escapeHtml(block);html=html.replace(/\[([^\]]+)\]\((https?:\/\/[^\s)]+)\)/g,(_all,label,url)=>`<a href="${url.replace(/&amp;/g,"&")}" target="_blank" rel="noopener noreferrer">${label}</a>`);html=html.replace(/`([^`]+)`/g,"<code>$1</code>").replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>").replace(/^((?:\[[A-Z0-9 _-]+\]){1,4})/gm,tags=>tags.replace(/\[([^\]]+)\]/g,'<span class="message-tag">$1</span>')).replace(/^###\s+(.+)$/gm,"<h4>$1</h4>").replace(/^##\s+(.+)$/gm,"<h3>$1</h3>").replace(/^#\s+(.+)$/gm,"<h2>$1</h2>");const lines=html.split("\n");let inList=false,out="";for(const line of lines){const match=line.match(/^\s*(?:[-*]|\d+\.)\s+(.+)$/);if(match){if(!inList){out+="<ul>";inList=true;}out+=`<li>${match[1]}</li>`;}else{if(inList){out+="</ul>";inList=false;}if(line)out+=`<p>${line}</p>`;} }if(inList)out+="</ul>";return out;}).join("");
}
...
function coordinationBundle(events){const profile=profileFor(events.at(-1).actor),actors=new Set(events.map(event=>event.actor)),key=`${events[0].id}-${events.at(-1).id}`;return `<details class="coordination-bundle" data-coordination-key="${key}" ${state.openCoordination.has(key)?"open":""}><summary><span class="coordination-avatar">${avatarContent(profile)}</span><span class="coordination-copy"><b>${actors.size===1?escapeHtml(profile.display_name||profile.name):"Team coordination"}</b><small>↔ ${events.length} resource update${events.length===1?"":"s"}</small></span><em>${state.openCoordination.has(key)?"Collapse":"Expand"}</em></summary><div>${events.map(event=>`<p><i class="${event.kind==="claim.acquired"?"acquired":"released"}"></i><b>${escapeHtml(profileFor(event.actor).display_name||event.actor)}</b><span>${escapeHtml(event.summary)}</span><time>${relative(event.created_at)}</time></p>`).join("")}</div></details>`;}
function renderTyping(raw=[]){const current=raw.filter(item=>item.channel===state.channel),active=new Set(current.map(item=>item.agent));for(const [agent,timer] of state.typingTimers)if(!active.has(agent)){clearTimeout(timer);state.typingTimers.delete(agent);}for(const agent of [...state.typingVisible])if(!active.has(agent))state.typingVisible.delete(agent);for(const item of current)if(!state.typingVisible.has(item.agent)&&!state.typingTimers.has(item.agent)){const timer=setTimeout(()=>{state.typingTimers.delete(item.agent);if((state.snapshot?.typing||[]).some(activeItem=>activeItem.agent===item.agent&&activeItem.channel===state.channel)){state.typingVisible.add(item.agent);renderTyping(state.snapshot.typing);}},300+Math.random()*1200);state.typingTimers.set(item.agent,timer);}const typing=current.filter(item=>state.typingVisible.has(item.agent)),typingNames=typing.map(item=>profileFor(item.agent).display_name||item.agent),singleAction=typing.length===1?(typing[0].stage==="reading"?"reading the chat":typing[0].action||"working on a reply"):"working on replies";$("#agent-typing").classList.toggle("hidden",!typing.length);$("#agent-typing").innerHTML=typing.length?`<span><i></i><i></i><i></i></span>${escapeHtml(typingNames.join(", "))} ${typing.length===1?"is":"are"} ${escapeHtml(singleAction)}…`:"";}
...
function render() {
  ...
  document.querySelectorAll("[data-channel]").forEach(el => el.onclick = async () => { state.channel = el.dataset.channel; if(state.channelKey)localStorage.setItem(state.channelKey,state.channel); document.querySelector(".sidebar")?.classList.remove("open"); await refresh(); });
...
function renderEvents(events) {
  ...
  timeline.innerHTML = compactTimeline(events).map(item=>{if(item.coordinationBundle)return coordinationBundle(item.coordinationBundle);const e=item,profile=profileFor(e.actor),parent=e.parent_id?byId.get(e.parent_id):null,parentProfile=parent?profileFor(parent.actor):null,attachments=Array.isArray(e.detail?.attachments)?e.detail.attachments:[];return `<article class="event ${e.actor_role}" data-event-id="${e.id}"><div class="event-avatar">${avatarContent(profile)}</div><div>${parent ? `<button class="reply-quote" data-jump="${parent.id}"><b>${escapeHtml(parentProfile.display_name || parent.actor)}</b><span>${escapeHtml(parent.summary)}</span></button>` : ""}<div class="event-head"><b>${escapeHtml(profile.display_name || e.actor)}</b><span class="role">${escapeHtml(e.actor_role)}</span><time>${relative(e.created_at)}</time><button class="pin-action ${pinned.has(e.id)?"pinned":""}" data-pin="${e.id}" title="${pinned.has(e.id)?"Unpin":"Pin as project guidance"}">${pinned.has(e.id)?"◆":"◇"}</button><button class="reply-action" data-reply="${e.id}" title="Reply">↩ Reply</button></div><div class="event-summary ${e.actor_role==="agent"?"rich-message":""}">${e.actor_role==="agent"?formatAgentMessage(e.summary):formatMessage(e.summary)}</div>${attachments.length?`<div class="event-attachments">${attachments.map(a=>`<button data-load-attachment="${escapeHtml(a.id)}" data-mime="${escapeHtml(a.mime_type)}" data-filename="${escapeHtml(a.filename)}"><b>${escapeHtml(a.filename)}</b><small>${formatBytes(a.size)} · click to open</small></button>`).join("")}</div>`:""}<div class="kind">${escapeHtml(e.kind)}${e.task_id ? ` · TASK #${e.task_id}` : ""}</div>${actionContext(e)}</div></article>`;}).join("");
  ...
  document.querySelectorAll("[data-jump]").forEach(button => button.onclick = () => { state.focusedEvent=Number(button.dataset.jump);state.focusUntil=Date.now()+5000;const target=document.querySelector(`[data-event-id="${state.focusedEvent}"]`);target?.scrollIntoView({behavior:"smooth",block:"center"});target?.classList.add("reply-flash");setTimeout(()=>{if(Date.now()>=state.focusUntil){state.focusedEvent=null;target?.classList.remove("reply-flash");}},5000); });
...
function renderActivities(activities) {
  ...
  $("#tasks").innerHTML=activities.length?activities.map(activity=>{const agent=profileFor(activity.agent),status=String(activity.status).replace("_"," "),timing=activity.status==="working"?`Working for ${elapsed(activity.started_at)}`:`${status} · updated ${elapsed(activity.updated_at)} ago`,editable=state.snapshot.me.editableProfiles.includes(activity.agent);return `<article class="task activity-card"><div class="task-top"><b>${escapeHtml(activity.title)}</b><i class="activity-dot status-${escapeHtml(activity.status)}"></i></div>${activity.description?`<p>${escapeHtml(activity.description)}</p>`:""}<div class="task-agent"><span class="task-agent-avatar">${avatarContent(agent)}</span><div><b>${escapeHtml(agent.display_name||agent.name)}</b><small class="status-${escapeHtml(activity.status)}">${escapeHtml(timing)}</small></div></div><div class="task-meta"><span class="task-status status-${escapeHtml(activity.status)}">${escapeHtml(status)}</span><span>#${escapeHtml(activity.channel)}</span></div>${editable?`<div class="activity-controls">${activity.status==="paused"?`<button data-activity-status="working" data-activity-agent="${escapeHtml(activity.agent)}">Resume</button>`:`<button data-activity-status="paused" data-activity-agent="${escapeHtml(activity.agent)}">Pause</button>`}<button data-activity-status="clear" data-activity-agent="${escapeHtml(activity.agent)}">Clear</button></div>`:""}</article>`;}).join(""):`<p class="no-items">No agents are actively working.</p>`;
  document.querySelectorAll("[data-activity-status]").forEach(button=>button.onclick=async()=>{await api(`/api/activities/${encodeURIComponent(button.dataset.activityAgent)}`,{method:"PATCH",body:JSON.stringify({status:button.dataset.activityStatus})});await refresh();});
}
...
$("#mobile-channels").onclick=()=>{document.querySelector(".sidebar").classList.toggle("open");$("#context-panel").classList.remove("open");};
$("#mobile-workboard").onclick=()=>{$("#context-panel").classList.toggle("open");document.querySelector(".sidebar").classList.remove("open");};
