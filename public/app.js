...
function render() {
  ...
  $("#self-avatar").textContent = initials(s.me.name);
  ...
  $("#presence").innerHTML = s.presence.slice(0, 8).map(p => `<div class="person"><div class="person-avatar">${initials(p.name)}<i></i></div><div><b>${escapeHtml(p.name)}</b><small>${escapeHtml(p.current_task || p.status)}</small></div></div>`).join("");
...
function renderEvents(events) {
  ...
  timeline.innerHTML = events.map(e => `<article class="event ${e.actor_role}"><div class="event-avatar">${initials(e.actor)}</div><div><div class="event-head"><b>${escapeHtml(e.actor)}</b><span class="role">${escapeHtml(e.actor_role)}</span><time>${relative(e.created_at)}</time></div><p class="event-summary">${escapeHtml(e.summary)}</p><div class="kind">${escapeHtml(e.kind)}${e.task_id ? ` · TASK #${e.task_id}` : ""}</div>${e.detail ? `<details class="context-card"><summary>Show action context</summary><pre>${escapeHtml(JSON.stringify(e.detail, null, 2))}</pre></details>` : ""}</div></article>`).join("");
