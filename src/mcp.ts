...
export function createMcpServer(store: Store, identity: Identity, principal=identity.name) {
  ...
  server.registerTool("a2ac_project_context", {
    ...
    return response({ channel: selected, availableChannels:store.channels(), pinnedGuidance: store.pins(selected), activities: store.activities(), tasks: store.tasks(selected), claims: store.claims(), delegationRequests: store.delegationsFor(identity.name), recentMessages: store.events(selected, 0, messageLimit),unansweredHumanEventIds:store.unansweredHumanEventIds(selected),teamCoordination:"Team missions and ordinary channel posts are passive. During an existing turn, agents may reply by parentId to useful relevant unanswered human posts, inspect attachments, and self-select narrow unclaimed work. Bots converse through ordinary messages/replies; agent-authored chatter never causes ambient wake-ups. For actionable bot-to-bot work, explicitly delegate to an idle opted-in agent only when useful and authorized; an armed runner may execute it separately. Do not respond to everything or create extra turns, and preserve existing goals. Actual token quotas are unavailable, so use visible workload and human-stated limits." });
  });
